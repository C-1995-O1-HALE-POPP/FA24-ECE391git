...
// the bit position for magic number to be tested
...
int elf_load(struct io_intf *io, void (**entryptr)(void)) {
    trace("%s(%p, %p)", __func__, io, entryptr);
    ...
    for (int i = 0; i < ehdr.e_phnum; i++) {
        // Check if the segment is of type PT_LOAD
        if (phdrs[i].p_type == PT_LOAD) {
            // move to offset
            if (ioseek(io, phdrs[i].p_offset) < 0) {
                kfree(phdrs);  
                kprintf("Failed to seek to segment offset\n");
                return -EIO;  // Return -EIO on seek error
            }
            // Check if the segment is within the user address space
            debug("Allocating memory for segment %d of size %u, property is %d\n", i, phdrs[i].p_memsz, phdrs[i].p_flags);
            if(phdrs[i].p_vaddr < USER_START_VMA || phdrs[i].p_vaddr + phdrs[i].p_memsz > USER_END_VMA){
                kprintf("Invalid segment address\n");
                kfree(phdrs);  
                return -EINVAL;
            }
            // check if the memory size is smaller than the file size
            if(phdrs[i].p_memsz < phdrs[i].p_filesz){
                kprintf("Invalid segment size\n");
                kfree(phdrs);  
                return -EINVAL;
            }
            ...
            memory_alloc_and_map_range(phdrs[i].p_vaddr, phdrs[i].p_memsz, PTE_R | PTE_W | PTE_U);
            memset((void*)phdrs[i].p_vaddr, 0, phdrs[i].p_memsz);

            // Load the segment content from the file into the allocated memory
            if (ioread_full(io, (void*)phdrs[i].p_vaddr, phdrs[i].p_filesz) != phdrs[i].p_filesz) {
                kfree(phdrs);    
                return -EIO;     // Return -EIO on read error
            }

            // set the flags for the segment
            memory_set_range_flags((void*)phdrs[i].p_vaddr, phdrs[i].p_memsz, pte_flags);
        }
    }

    kfree(phdrs);
    kprintf("ELF loaded successfully, exe_entry is 0x%x\n", *entryptr);
