...
_thread_finish_jump:
        ...
        # intr_disable
        csrr   t0, sstatus      # set sstatus.SIE = 0
        li     t1, ~(1<<1)      # SIE is sstatus[1]
        and    t0, t0, t1
        csrw   sstatus, t0

        csrw   sscratch, a0     # Save stack anchor in sscratch

        la     t0, _trap_entry_from_umode
        csrw   stvec, t0        # Set trap vector to umode

        csrr   t0, sstatus      # set sstatus.SPP = 0
        li     t1, ~(1<<8)      # SPP is sstatus[8]
        and    t0, t0, t1

        li     t1, 1<<5         # SPIE is sstatus[5]
        or     t0, t0, t1       # Set SPIE to 1
        csrw   sstatus, t0

        csrw   sepc, a2         # Set user pc
        mv     sp, a1           # Set user stack pointer

        # intr_enable
        csrr   t0, sstatus      # set sstatus.SIE = 1
        li     t1, 1<<1         # SIE is sstatus[1]
        or     t0, t0, t1
        csrw   sstatus, t0
