...
void plic_set_source_priority(uint32_t srcno, uint32_t level) {
    ...
    #endif
    volatile uint32_t* dst = (volatile uint32_t*)((uintptr_t)(srcno * 4 + PLIC_IOBASE));
    *dst = level;
...
int plic_source_pending(uint32_t srcno) {
    ...
    #endif
    volatile uint32_t* array = ((volatile uint32_t*)((uintptr_t)(PENDING_BASE + 4*(srcno/32))));
    return (1 & (*array >> (srcno%32))) != 0;
}
...
void plic_enable_source_for_context(uint32_t ctxno, uint32_t srcno) {
    ...
    volatile uint32_t* src_array_loc = (volatile uint32_t*)((uintptr_t)(ctx_base_loc + 4*(srcno/32)));
    *src_array_loc |= (1 << (srcno % 32));
}
...
void plic_disable_source_for_context(uint32_t ctxno, uint32_t srcid) {
    ...
    #endif
    volatile uint32_t* ctx_base_loc = (volatile uint32_t*)((uintptr_t)(ENABLE_BASE + ctxno * ENABLE_SIZE_PER_CONTEXT));
    volatile uint32_t* src_array_loc = (volatile uint32_t*)((uintptr_t)(ctx_base_loc + 4*(srcid/32)));
    *src_array_loc &= ~(1 << (srcid % 32));
}
