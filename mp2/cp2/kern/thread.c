...
int thread_spawn(const char * name, void (*start)(void *), void * arg) {
    struct thread * child;
    int tid;
    trace("%s(name=\"%s\") in %s", __func__, name, running_thread());
    ...
    //console_printf("Thread %d (%s) spawned\n", tid, name);
    return tid;
}

void thread_exit(void) {
    //console_printf("Thread %d(%s) is exiting\n", CURTHR->id, CURTHR->name);
...
int thread_join(int tid) {
    ...
    // input: tid
    // output: thread id of the child
    // side effect: Waits for a specific child thread to exit.
    //console_printf("Thread %d(%s) is waiting for %d\n", CURTHR->id, CURTHR->name, tid);
    if(tid < 1 || tid >= NTHR) {
        return -1;
    }
    struct thread* child = thrtab[tid];
    // if child thread valid
    if (child == NULL || child->parent != CURTHR) {
        return -1;
    }
    // if child thread has exited, recycle the thread immediately
    if (child->state == THREAD_EXITED) {
        recycle_thread(tid);
        return tid;
    }else{
        // if child thread has not exited, wait for it to exit
...
void condition_wait(struct condition * cond) {
    ...
    //console_printf("Thread %d(%s) is waiting on %s\n", CURTHR->id, CURTHR->name, cond->name);
...
void condition_broadcast(struct condition * cond) {
    ...
    // output: none
    // side effect: Signals (wakes up) all threads waiting on the condition
    //for(int tid = 1; tid < NTHR; tid++) {
    //    if (thrtab[tid] != NULL) {
    //        console_printf("Thread %d(%s) is in condition %d\n", tid, thrtab[tid]->name, thrtab[tid]->state);
    //    }
    //}
    
    int s = intr_disable();
    //console_printf("Broadcast (%s)\n", cond->name);
    while (!tlempty(&cond->wait_list)) {
        ...
        // Placing it on the ready-to-run list
        tlinsert(&ready_list, thr);
        //console_printf("Wake up %d\n", cond->name, thr->id);
    }
    intr_restore(s);
}
...
void recycle_thread(int tid) {
    ...
    int ctid;
    //console_printf("Recycling thread %d\n", ctid);
...
void suspend_self(void) {
    ...
    // input: none
    // output: none
    // side effect: Suspends the currently running thread, put it in the ready list
    //              resumes the next thread on the ready list

    // if runnable, inserted at the tail of ready list
    assert(intr_enabled());
    ...
    //console_printf("Thread %d(%s) is suspended\n", CURTHR->id, CURTHR->name);
